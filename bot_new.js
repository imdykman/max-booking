...
async function sendWelcome(ctx) {
    ...
    await ctx.reply(
        `🎓 *ОТДИС — Приёмная комиссия*\n\n👋 *Добро пожаловать, ${userName}!*\n\nЯ — официальный помощник. Выберите вопрос или откройте приложение:`,
        { attachments: [appKeyboard] }
    );
    
    await ctx.reply('👇 *Выберите вопрос:*', { attachments: [getMainKeyboard()] });
}
...
Бюджетные места: Конструирование, Дизайн (легкая промышленность), Мастер швейных изделий, Оператор швейного оборудования, Художник по костюму, Декоративно-прикладное искусство
